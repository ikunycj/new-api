...
func TestGetFailoverMonitoringSnapshotAggregatesMetricsAndAlerts(t *testing.T) {
	prometheus := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		values := map[string]string{
			...
			`sum(increase(new_api_routing_channel_switch_total[5m]))`:       "3",
			`sum(new_api_routing_channel_circuit_state{state="open"} == 1)`: "1",
...
func TestGetFailoverMonitoringSnapshotReturnsChannelAlerts(t *testing.T) {
	prometheus := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		expectedQueries := map[string]struct{}{
			...
			`sum(increase(new_api_routing_channel_switch_total[5m]))`:       {},
			`sum(new_api_routing_channel_circuit_state{state="open"} == 1)`: {},
