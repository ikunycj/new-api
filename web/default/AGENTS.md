...
- **管理员页面与功能例外**：仅管理员/Root 可访问的页面和功能只需提供中文文案，不要求补充或维护其他语言，也不必为此新增 locale key 或运行多语言同步。若组件被普通用户页面复用，或文案出现在管理员与普通用户共用的界面中，仍按完整 i18n 规则处理。
- **使用场景**
...
- **分支端口**：`ikun.love` 分支使用 `http://localhost:5174`；主样式分支（`feature/docs-perfect`）使用 `http://localhost:5173`；未指定分支时使用 `http://localhost:5713`。
- **禁止端口漂移**：不得通过 `--port` 改用其他端口，也不得在分配端口被占用时自动切换。`rsbuild.config.ts` 必须保持 `strictPort: true`；端口通过当前分支对应的启动配置传入。
- **启动前检查**：先检查当前分支分配的端口是否已经由本工作区的健康前端服务监听；若是则直接复用，不重复启动。
- **冲突处理**：若分配端口被无关或异常进程占用，先确认 PID 和进程来源并报告冲突。未经明确授权，不得结束用户进程或临时换端口规避冲突。
- **按需启动**：只运行测试、类型检查、lint、format 或 production build 时不启动开发服务器；需要浏览器验收或用户预览时才启动，并提供当前分支对应的 URL。
- **后端约定**：本地 Go 后端固定使用 `http://127.0.0.1:3000`，Rsbuild 的 `/api`、`/mj`、`/pg` 代理指向该服务。
...
- **2026-07-27**：固定前端开发服务器端口为 5173，禁止端口自动漂移，并明确启动复用与冲突处理规范。
