...
- **固定端口**：默认前端开发服务器固定为 `http://localhost:5173`，统一从 `web/default/` 执行 `bun run dev`。
- **禁止端口漂移**：不得通过 `--port` 改用其他端口，也不得在 `5173` 被占用时自动切换到 `3001`、`5174` 等端口。`rsbuild.config.ts` 必须保持 `port: 5173` 与 `strictPort: true`。
- **启动前检查**：先检查 `5173` 是否已经由本工作区的健康前端服务监听；若是则直接复用，不重复启动。
- **冲突处理**：若 `5173` 被无关或异常进程占用，先确认 PID 和进程来源并报告冲突。未经明确授权，不得结束用户进程，也不得临时换端口规避冲突。
- **按需启动**：只运行测试、类型检查、lint、format 或 production build 时不启动开发服务器；需要浏览器验收或用户预览时才启动，并统一提供 `http://localhost:5173`。
