...
将 API Key 直接写入用户级 `~/.hermes/config.yaml`（Windows 对应 `%USERPROFILE%\.hermes\config.yaml`）。关闭终端后，Hermes 仍会从该配置文件读取密钥。该文件包含明文密钥，请限制访问权限，不要提交到 Git。
...
- 检查 `providers.alltokenapi.api_key` 是否已填写有效密钥。
