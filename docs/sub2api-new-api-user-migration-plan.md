...
本文只覆盖普通用户、余额、源端订阅剩余额度折算和 API Key。目标系统不创建订阅，源端剩余额度只会折算后并入钱包。日志、请求记录、登录会话和
...
| 源端有效订阅剩余额度 | `quota` | 按第 4 节折算后与钱包余额相加，不创建目标订阅 |
...
只有未软删除、状态为 `active`、未过期且所属源分组可以映射到目标计费分组的源端订阅参与余额折算。目标系统不保留订阅实例。源端订阅没有
...
2. 校验目标分组、邮箱冲突、数据库字段容量、冻结余额、源端订阅窗口和 Key 格式。
