...
字段由 GORM `AutoMigrate(&User{})` 管理，兼容 SQLite、MySQL 和 PostgreSQL。本地开发进程与 `aliyun` 测试服务都以 `NODE_TYPE=master` 运行并连接同一套测试数据库，因此两者都可能执行迁移；部署后应检查共享测试库的 schema 和数据状态。生产数据库与这套测试数据库保持隔离。
...
- 为 SQLite/MySQL/PostgreSQL 验证 AutoMigrate；在双 Master 测试环境部署后检查共享 schema，再进行引导流程验收。
