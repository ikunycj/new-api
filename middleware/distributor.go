...
func isPlaygroundRequest(path string) bool {
	...
	return func(c *gin.Context) {
		...
		if ok {
			...
			if shouldSelectChannel {
				...
				if isPlaygroundRequest(c.Request.URL.Path) {
					...
					if playgroundRequest.Group != "" {
						if !service.UserGroupCanUsePricingGroup(userGroup, playgroundRequest.Group) && playgroundRequest.Group != usingGroup {
				...
				// Affinity may choose a channel only inside the first usable ordered
				// candidate. A cached channel in a later group must never bypass an
				// earlier candidate with the same model name.
