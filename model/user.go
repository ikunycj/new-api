...
// current onboarding flow without exposing a writable pointer to callers.
func NewUserOnboardingVersion() *int {
	version := OnboardingPendingVersion
	return &version
}
...
func (user *User) OnboardingStatus() UserOnboardingStatus {
	status := UserOnboardingStatus{Version: user.OnboardingVersion}
	status.Required = user.OnboardingVersion != nil && *user.OnboardingVersion < CurrentOnboardingVersion
	return status
}
...
// CompleteUserOnboarding advances an enrolled user to the current onboarding
// version. The conditional update keeps the operation idempotent and leaves
// legacy or admin-created users with a NULL version unchanged.
func CompleteUserOnboarding(id int) (*User, error) {
	if id == 0 {
		return nil, errors.New("id 为空！")
	}
	if err := DB.Model(&User{}).
		Where("id = ? AND onboarding_version IS NOT NULL AND onboarding_version < ?", id, CurrentOnboardingVersion).
		Update("onboarding_version", CurrentOnboardingVersion).Error; err != nil {
		return nil, err
	}
