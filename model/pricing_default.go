...
// defaultVendorRules is ordered because some model names match more than one
// provider keyword. Earlier rules take precedence over later rules.
...
func initDefaultVendorMapping(metaMap map[string]*Model, vendorMap map[int]*Vendor, enableAbilities []AbilityWithChannel) {
	for _, ability := range enableAbilities {
		...
		// 匹配供应商
		vendorID := 0
		if vendorName := inferDefaultVendor(modelName); vendorName != "" {
...
func inferDefaultVendor(modelName string) string {
